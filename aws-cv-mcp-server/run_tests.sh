...
# Check if aws_cv_mcp_server module can be imported
echo "Checking if aws_cv_mcp_server module can be imported..."
python -c "import awslabs.aws_cv_mcp_server; print(f'aws_cv_mcp_server module found at: {awslabs.aws_cv_mcp_server.__file__}')" 2>/dev/null
if [ $? -ne 0 ]; then
    echo "Warning: aws_cv_mcp_server module cannot be imported. This may cause test failures."
    ...
    # Check if pip is available
        ...
        ln -sf $(pwd)/awslabs/aws_cv_mcp_server $(pwd)/aws_cv_mcp_server 2>/dev/null
    fi

    echo "Trying again..."
    python -c "import awslabs.aws_cv_mcp_server; print(f'aws_cv_mcp_server module found at: {awslabs.aws_cv_mcp_server.__file__}')" 2>/dev/null
    if [ $? -ne 0 ]; then
        echo "Still cannot import aws_cv_mcp_server module. Tests may fail."
...
# $PYTEST_CMD --cov=aws_cv_mcp_server --cov-report=term-missing tests/

# If you want to run with coverage and generate an HTML report, uncomment the following line
# $PYTEST_CMD --cov=aws_cv_mcp_server --cov-report=html tests/
