...
```json
{
  "mcpServers": {
    "awslabs.aws-cv-mcp-server": {
      "command": "uvx",
      "args": ["awslabs.aws-cv-mcp-server"],
      "env": {
...
or docker after a succesful `docker build -t awslabs/aws-cv-mcp-server .`:
...
```bash
pytest --cov=awslabs.aws_cv_mcp_server --cov-report=term-missing tests/
```
